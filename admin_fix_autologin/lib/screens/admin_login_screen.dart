...
class _AdminLoginScreenState extends State<AdminLoginScreen>
  ...
  Future<void> _checkAutoLogin() async {
    final prefs = await SharedPreferences.getInstance();
    final savedCnic = prefs.getString('admin_cnic') ?? '';
    final savedPass = prefs.getString('admin_password') ?? '';
    if (savedCnic.isEmpty || savedPass.isEmpty) return;
    if (!mounted) return;
    // Auto-login with saved credentials
    setState(() { _loading = true; });
    final result = await _svc.loginWithCredentials(savedCnic, savedPass);
    if (!mounted) return;
    if (result == null) {
    ...
    // Saved credentials invalid — clear and show login form
